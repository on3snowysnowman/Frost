...
public:
     ...
     */
    void set_sprite_position(const sprite_id id, uint16_t x, uint16_t y);
    ...
    */
    const sprite_id create_sprite(uint16_t splice_x, uint16_t splice_y, uint16_t splice_w, 
...
private:
     ...
     * ID. Heap allocated. */
    static std::vector<Sprite>* const s_all_sprites;
    ...
    /** Tracks the number of Sprites that require each Texture. Heap allocated. 
     */
     ...
     */
    void _place_id_in_rendering_ids(const sprite_id id);

    /** @brief Removes the id from the m_sprites_to_render vector.
     * 
     * @param id ID to remove.
     */
    void _remove_id_from_rendering_ids(const sprite_id id);

    /** Returns true if the Sprite assigned to the passed ID is existant and available. 
     * 
     * @param id ID to check.
    */
    static bool _is_id_valid(const sprite_id id);
     ...
     */
    static const sprite_id _get_next_id();
};
